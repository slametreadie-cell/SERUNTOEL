...
export function exportPDF(judul, kolom, rows, opts = {}) {
  ...
  const body = rows.map((r) => `<tr>${kolom.map((k) => {
    const v = r[k.key];
    const align = typeof v === "number" || /rp|total|jumlah|omset|harga|hpp|laba|diskon/i.test(k.key) ? "right" : "left";
    const teks = typeof v === "number" && /rp|total|jumlah|omset|harga|hpp|laba|diskon|biaya|pendapatan|budget|target/i.test(k.key) ? rp(v) : v ?? "\u2014";
    return `<td class="${align}">${esc(teks)}</td>`;
  }).join("")}</tr>`).join("");
  const ringkas = (opts.ringkasan || []).map((x) => `<div class="sum"><span>${esc(x.label)}</span><b>${esc(x.value)}</b></div>`).join("");
...
<style>
  * { box-sizing: border-box; }
  ...
  <h1>${esc(judul)}</h1>
  <div class="sub">${esc(opts.subjudul || "")} \xB7 Dicetak ${(/* @__PURE__ */ new Date()).toLocaleString("id-ID")}</div>
  ...
  <div class="actions">
    <button class="primary" onclick="window.print()">\u{1F5A8}\uFE0F Simpan sebagai PDF</button>
